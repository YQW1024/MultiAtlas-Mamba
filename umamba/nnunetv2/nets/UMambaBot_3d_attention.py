...
class ChannelAttention(nn.Module):
    ...
    def forward(self, x):
     ...
     #       print(f"  Sample {b}: ", att.cpu().detach().numpy())
        ...
        out = x * attention                           # 加权特征图
        weights = attention.view(x.shape[0], -1)      # 提取 [B, C]
        return out, weights
...
class BasicResBlock(nn.Module):
    def __init__(
            ...
            use_channel_attention=False  # 是否使用通道注意力的标志jia
        ):
        super().__init__()
...
class UNetResEncoder(nn.Module):
    ...
    def forward(self, x):
        if self.stem is not None:
            x = self.stem(x)
         #   print(f"Stem Output: {x.shape}")  # 打印 stem 输出的形状jia
        ret = []
        for s in self.stages:
            x = s(x)
         #   print(f"Stage Output: {x.shape}")  # 打印每个阶段的输出形状jia
            ret.append(x)
...
class UMambaBot(nn.Module):
    def __init__(self,
        ...
        self.encoder = UNetResEncoder(
            ...
            use_channel_attention=True#jia
        )
...
def get_umamba_bot_3d_from_plans(
   ...
   # print("input_channels:", num_input_channels)


    model = network_class(
        input_channels=num_input_channels,
      #  input_channels=6,#gai
        n_stages=num_stages,
