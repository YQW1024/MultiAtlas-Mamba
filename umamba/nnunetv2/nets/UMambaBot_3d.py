...
class UMambaBot(nn.Module):
    def __init__(self,
                 ...
                 num_classes_list: List[int],  # 修改：传入 6 个任务的类别列表
        ...
        self.decoders = nn.ModuleList([
            UNetResDecoder(self.encoder, nc, n_conv_per_stage_decoder, deep_supervision)
    ...
    def forward(self, x):
        ...
        # 返回 6 个任务的输出列表
        # 每个 decoder 输出可能是单个 Tensor (无深监督) 或 Tensor 列表 (有深监督)
        return [decoder(skips) for decoder in self.decoders]

    def compute_conv_feature_map_size(self, input_size):
        # 修复：总大小 = 1个Encoder + 所有Decoder的大小总和
        if not hasattr(self.encoder, 'conv_op'): # 兼容性检查
            conv_op = convert_dim_to_conv_op(len(input_size))
        else:
            conv_op = self.encoder.conv_op
            
        assert len(input_size) == convert_conv_op_to_dim(conv_op)
        
        size = self.encoder.compute_conv_feature_map_size(input_size)
        for decoder in self.decoders:
            size += decoder.compute_conv_feature_map_size(input_size)
        return size
...
def get_umamba_bot_3d_from_plans(
        ...
        # 默认这里可以写死或者从外部传入 6 个脑区任务的类别数
        num_classes_list: List[int] = None 
    ):
    # 如果没有传入，默认设置你的 6 个任务类别（请根据实际情况修改数值）
    # 类别数 = 脑区数 + 1 (背景)
