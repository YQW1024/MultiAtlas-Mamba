...
#    print(f"DEBUG: Unique labels in segmentation: {np.unique(segmentation.cpu().numpy())}")
#    print(f"DEBUG: Segmentation Max: {segmentation.max()}")
#    print(f"DEBUG: Label Manager Foreground Labels Count: {len(label_manager.foreground_labels)}")
...
#                                              dtype=np.uint8 if len(label_manager.foreground_labels) < 255 else np.uint16)#yuan
                                              
    segmentation_reverted_cropping = np.zeros(properties_dict['shape_before_cropping'], dtype=np.uint16) #1                                         
...
def export_prediction_from_logits(predicted_array_or_file: Union[np.ndarray, torch.Tensor], properties_dict: dict,
    ...
    segmentation_final = segmentation_final.astype(np.uint16)#1
...
def resample_and_save(predicted: Union[torch.Tensor, np.ndarray], target_shape: List[int], output_file: str,
    ...
    np.savez_compressed(output_file, seg=segmentation.astype(np.uint16))#1
    
#    np.savez_compressed(output_file, seg=segmentation.astype(np.uint8))#yuan
    torch.set_num_threads(old_threads)
