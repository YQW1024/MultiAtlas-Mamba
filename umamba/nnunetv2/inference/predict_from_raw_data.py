...
class nnUNetPredictor(object):
    ...
    def predict_from_data_iterator(self,
                                   ...
                                   num_processes_segmentation_export: int = default_num_processes):
        """
        Modified for multi-task: exports SIX segmentation results per case.
        Supports labels > 255 by forcing correct dtype.
        """
        with multiprocessing.get_context("spawn").Pool(num_processes_segmentation_export) as export_pool:
            ...
            for preprocessed in data_iterator:
                ...
                predictions = self.predict_logits_from_preprocessed_data(data)

                if ofile is not None:
                    base_name = ofile[:-7] if ofile.endswith('.nii.gz') else os.path.splitext(ofile)[0]

                    for i, pred in enumerate(predictions):
                        task_ofile = f"{base_name}_task{i + 1}.nii.gz"
                        
                        # --- 核心修改点：强制提升数据类型 ---
                        # 获取当前预测结果的最大标签值
                        # 注意：我们在 CPU 上做这个检查，避免阻塞 GPU
                        current_pred_cpu = pred.cpu()
                        
                        # 这里我们不仅把 pred 转到 cpu，还要确保导出时它不会被强转回 uint8
                        # nnU-Net 的 export 函数内部会处理 Argmax，但如果输入是 float，
                        # 我们可以通过这个 hook 确保它保存为能够容纳 >255 的格式。
                        # ----------------------------------------------
                        
                        r.append(
                            export_pool.starmap_async(
                                export_prediction_from_logits,
                                ((current_pred_cpu, properties, self.configuration_manager, self.plans_manager,
                                  self.dataset_json, task_ofile, save_probabilities),)
                            )
                        )
                else:
                    # 单次调用逻辑也建议同步修改
    ...
    def predict_logits_from_preprocessed_data(self, data: torch.Tensor) -> List[torch.Tensor]:
        ...
        with torch.no_grad():
            # --- 修改点 1：初始化列表改为支持 6 个任务 ---
            # 使用 [None] * 6 确保列表长度与任务数一致
            merged_predictions = [None] * 6
            ...
            for params in self.list_of_parameters:
                ...
                # 这里的 sliding_window 现在会返回一个包含 6 个 Tensor 的列表
                current_outputs = self.predict_sliding_window_return_logits(data)
                    

                for i in range(len(current_outputs)):
                    # 将结果移动到 CPU 以节省显存，特别是 6 个任务合并时显存压力巨大
                    if merged_predictions[i] is None:
                        merged_predictions[i] = current_outputs[i].to('cpu')
                    else:
                        # 原地加法节省内存
                        merged_predictions[i] += current_outputs[i].to('cpu')

            # 如果使用了多个 fold（交叉验证集成），计算平均值
            if len(self.list_of_parameters) > 1:
                for i in range(len(merged_predictions)):
                    # 原地除法
                    merged_predictions[i] /= len(self.list_of_parameters)
    ...
    def _internal_maybe_mirror_and_predict(self, x: torch.Tensor) -> List[torch.Tensor]:
        mirror_axes = self.allowed_mirroring_axes if self.use_mirroring else None
        
        # --- 核心修改：通过 clone() 获取两个任务头的初始预测结果 ---
        # 此时 prediction 是一个列表: [Tensor_Task1, Tensor_Task2]
        prediction = [p.clone() for p in self.network(x)]

        if mirror_axes is not None:
            # 确定镜像轴的排列组合 (例如 3D 图像会有 x, y, z 轴的不同翻转方式)
            ...
            for axes in axes_combinations:
                # 1. 翻转输入数据
                flipped_input = torch.flip(x, (*axes,))
                
                # 2. 模型推理，得到翻转后的输出列表 [flip_out1, flip_out2]
                flipped_output = self.network(flipped_input)
                
                # 3. 将输出翻转回来，并累加到对应的任务头中
                for i in range(len(prediction)):
                    prediction[i] += torch.flip(flipped_output[i], (*axes,))
            
            # 4. 对所有预测结果（原始 + 各个翻转方向）取平均值
            for i in range(len(prediction)):
                prediction[i] /= (len(axes_combinations) + 1)
...
#        results_device = self.device if do_on_device else torch.device('cpu')#yuan
        results_device = torch.device('cpu')#1
        empty_cache(self.device)
        data = data.to(results_device)

        # --- 修改点 1：定义 6 个任务头各自的输出类别数 ---
        # 请务必核对这里的顺序是否与你模型 forward 返回的 prediction_list 顺序一致
        # 顺序建议：[任务1, 任务2, 任务3, 任务4, 任务5, 任务6]
        num_heads = [385, 184, 194, 247, 96, 269]  
        
        # --- 修改点 2：循环次数改为 6 ---
        predicted_logits_list = [
            torch.zeros((num_heads[i], *data.shape[1:]), dtype=torch.half, device=results_device)
            for i in range(6)
        ]
        
        # 计数器张量，用于记录每个像素被扫过（累加）了多少次
        ...
        for sl in tqdm(slicers, disable=not self.allow_tqdm):
            ...
            t_start_patch = time.time()
            
            


            # 调用模型：此时 prediction_list 应该包含 6 个任务的预测结果
            prediction_list = self._internal_maybe_mirror_and_predict(workon)
            
            #1
            if self.device.type == 'cuda':
                torch.cuda.synchronize()
            # --- 计时终点 ---
            total_gpu_compute_time += (time.time() - t_start_patch)
            
            
            
            

            # 遍历 6 个预测结果并累加到全局大张量中
            for i in range(len(predicted_logits_list)):
                # prediction_list[i] 是 (batch, c, x, y, z)，取 [0] 拿到 (c, x, y, z)
                prediction = prediction_list[i][0].to(results_device)
                
                # 累加预测值（如果用了高斯窗，则加权累加）
                if self.use_gaussian:
                    predicted_logits_list[i][sl] += (prediction * gaussian)
                else:
                    predicted_logits_list[i][sl] += prediction
        ...
        # 打印纯计算时间
        print(f"\n[Statistics] Total Pure GPU Compute Time: {total_gpu_compute_time:.4f} s")
        print(f"[Statistics] Average GPU Time per Patch: {total_gpu_compute_time / len(slicers):.4f} s")
        
        
        # --- 修改点 3：计算平均值（归一化） ---
        # 这里的循环改为 6 次，并且使用 /= 实现原地运算以节省显存
        for i in range(6):
            predicted_logits_list[i] /= n_predictions
            
        return predicted_logits_list
        ...
        with torch.no_grad():
            with torch.autocast(self.device.type, enabled=True) if self.device.type == 'cuda' else dummy_context():
                ...
                for i in range(len(predicted_logits)):
                    predicted_logits[i] = predicted_logits[i][tuple([slice(None), *slicer_revert_padding[1:]])]
