...
class nnUNetPredictor(object):
    ...
    def predict_from_data_iterator(self,
                                   ...
                                   num_processes_segmentation_export: int = default_num_processes):
        """
        Modified for multi-task: exports two segmentation results per case.
        """
        with multiprocessing.get_context("spawn").Pool(num_processes_segmentation_export) as export_pool:
            ...
            for preprocessed in data_iterator:
                ...
                if ofile is not None:
                    print(f'\nPredicting {os.path.basename(ofile)} (Multi-task Mode):')
                else:
                ...
                # --- 1. 获取预测结果列表 [pred_184, pred_90] ---
                # 注意：确保你的 predict_logits_from_preprocessed_data 返回的是 List
                predictions = self.predict_logits_from_preprocessed_data(data)


                if ofile is not None:
                    # 1. 强制获取基础文件名（去掉 .nii.gz）
                    # nnU-Net 的 ofile 可能是 'case_001.nii.gz'
                    if ofile.endswith('.nii.gz'):
                        base_name = ofile[:-7]
                    else:
                        base_name = os.path.splitext(ofile)[0]

                    # 2. 手动拼接清晰的任务后缀
                    ofile_task1 = f"{base_name}_task1.nii.gz"
                    ofile_task2 = f"{base_name}_task2.nii.gz"

                    print(f'Sending off Tasks to background workers...')
                    print(f'Task 1 path: {os.path.basename(ofile_task1)}')
                    print(f'Task 2 path: {os.path.basename(ofile_task2)}')

                    # 导出任务 1 (194区)
                    r.append(
                        export_pool.starmap_async(
                            export_prediction_from_logits,
                            ((predictions[0].cpu(), properties, self.configuration_manager, self.plans_manager,
                              self.dataset_json, ofile_task1, save_probabilities),)
                        )
                    )
                    ...
                    r.append(
                        export_pool.starmap_async(
                            export_prediction_from_logits,
                            ((predictions[1].cpu(), properties, self.configuration_manager, self.plans_manager,
                              self.dataset_json, ofile_task2, save_probabilities),)
                        )
                    )
                else:
                    # 如果没有指定输出文件（单次调用情况）
    ...
    def predict_logits_from_preprocessed_data(self, data: torch.Tensor) -> List[torch.Tensor]:
        ...
        with torch.no_grad():
            ...
            for params in self.list_of_parameters:
                ...
                # 这里的 sliding_window 会返回一个列表 [logits_184, logits_90]
    ...
    def _internal_predict_sliding_window_return_logits(self,
        ...
        # 任务 1: 185 类 (184+1)
        # 任务 2: 91 类 (90+1)
        # 如果你想动态获取，可以使用 self.network 的输出通道定义
        num_heads = [194, 184] 
        ...
        for sl in tqdm(slicers, disable=not self.allow_tqdm):
            ...
            for i in range(len(predicted_logits_list)):
                ...
                # --- 核心修改：先用 [i] 拿到 Tensor，再用 [sl] 切片 ---
                predicted_logits_list[i][sl] += (prediction * gaussian if self.use_gaussian else prediction)
